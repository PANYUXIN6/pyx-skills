...
Use the complete target document, Contract Ledger, complete declared authority files, and observed repository context. Trace module entry points, dependency direction, ownership, version mirrors, transaction boundaries, concurrency, lifecycle, and error propagation. For child-task designs, trace declared responsibilities and handoffs against the confirmed governing authority and current repository ownership. Use observed context only to establish current repository facts; derive expected behavior only from the target or confirmed authority. Produce only cross-boundary contract violations with a concrete finite trigger path.
