...
The Runner selects `full` without a model when any accepted Evidence Card is architectural, a non-target review document or config changed, the Markdown preamble changed, an accepted contract heading is ambiguous or changes level, an existing heading is removed, renamed, or reordered, or content or headings change outside accepted contract subtrees. A unique accepted self-consistency contract may change its own content and descendant content, and may add new descendant headings; it may not add a sibling contract. The bounded verification task remains responsible for escalating direct repair interactions that create or change another contract or ownership boundary.
