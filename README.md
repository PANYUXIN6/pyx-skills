...
| [`code-review`](./code-review/) | 以 Skill 路由语义审查，以确定性 Runner 冻结输入、校验候选锚点、强制逐项挑战裁决并约束结论。 | 仅在用户明确要求发现问题、评估质量、验证正确性或生成审查结论时触发；阅读、解释、梳理和追踪实现不进入审查。只有确认成立的 Finding 进入最终结果，默认只报告、不自动修复。 |
