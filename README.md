...
- **职责**：先检查项目事实，再按风险选择 Fast Path、Design Brief 或 Full Design；研究成熟方案，收敛关键决策、责任边界、失败行为和验证策略。
- **产出**：小任务可以直接进入实现；中等不确定性得到可确认的设计简报；高影响任务得到经确认且可持久化的设计文档。依赖任务进一步形成 governing design 和可靠实施切片，使另一个 Agent 能在不重做架构推理的情况下执行。
- **边界**：不为清晰、局部、可逆任务制造设计流程；不生成稻草人方案；不把实施切片写成缺少仓库证据的文件级伪代码，也不强制设计文档永久保留。

### [`repo-map-first`](./repo-map-first/)

- **触发**：代码归属不明、变更跨越职责边界、入口或依赖方向可能改变、仓库不熟悉，或相关地图缺失、陈旧、矛盾；用户显式要求地图工作时始终执行完整地图流程。
- **职责**：从规则、源码、入口、调用链、依赖和测试中确定正确落点；区分 confirmed contract 与 `authority_status: observed` 的观察性地图；为 dependent skill 引导或验证必要的仓库上下文。
- **产出**：简明的落点判断和证据；显式地图模式下创建或修复 `docs/REPO_MAP.md` 与 `docs/ARCHITECTURE.md` 的相关范围；结构发生变化时同步地图。
